...
gatk VariantFiltration \
  ...
  -O ${output}_GQ${GQ}_DP${DP}_MISS.vcf.gz \
  --java-options "-Xmx12g" \
  --filter-expression "AN < $MIN_NUMBER_OF_ALLELES" \
  --filter-name "MISS" \
  --missing-values-evaluate-as-failing &&
...
gatk SelectVariants \
  -R $hg38ref \
  -V ${output}_GQ${GQ}_DP${DP}_MISS.vcf.gz \
  -O ${output}_GQ${GQ}_DP${DP}_MISS_filtered.vcf.gz \
  --java-options "-Xmx12g" \
  --exclude-filtered \
...
rm ${output}_GQ${GQ}_DP${DP}_MISS.vcf.gz ${output}_GQ${GQ}_DP${DP}_MISS.vcf.gz.tbi &&
